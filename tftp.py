...
opcode_names = ( "(unknown)", "Read Request", "Write Request", "Data", "Acknowledgement", "Error" )
...
class Request( Packet ) :
	...
	def pack( self ) :
		self.packet = struct.pack( ("!H%dsc%dsc"%(len(self.filename),len(self.mode))), \
				self.op, self.filename, "\x00", self.mode, "\x00" )
...
class Ack( Packet ) :
	...
	def unpack( self ) :
		...
		if len(self.packet) < 4 :
			raise packet_error( self.packet, "Packet too small (len=%d)."%len(self.packet) )
		self.op,self.block_num,junk = struct.unpack( ("!HH%ds" % (len(self.packet)-4) ), self.packet )
...
class Data( Packet ) :
	...
	def pack( self ) :
		...
		self.packet = struct.pack( ("!HH%ds"%len(self.data)), self.op, self.block_num, self.data )

	def unpack( self ) :
		"""Parse a DATA, verifying pkt, extracting data field and block number."""
		if len(self.packet) > 516 :
			raise packet_error( self.packet, "Packet too large (len=%d)."%len(self.packet) )
		self.op,self.block_num,self.data = struct.unpack( ("!HH%ds" % (len(self.packet)-4)), self.packet )
...
class Error( Packet ) :
	...
	def pack( self ) :
		self.packet = struct.pack( ("!HH%dsc"%len(self.error_msg)), self.op, self.error_code, self.error_msg, "\x00" )

	def unpack( self ) :
		self.op,self.error_code,error_msg = struct.unpack( ("!HH%ds" % (len(self.packet)-4)), self.packet )
		# take up to the first NULL as the error message
		self.error_msg = error_msg.split( "\x00" ) [0]
